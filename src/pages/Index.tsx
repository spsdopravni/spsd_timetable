...
const Index = () => {
  ...
  const renderUnifiedHeader = () => (
    ...
    >
      ...
      <div className="px-6 py-4 relative z-10 h-full flex items-center justify-between">
        <div className="flex items-center gap-6">
          <div className="flex-shrink-0 cursor-pointer" onClick={handleLogoClick}>
            <img 
              src="/pictures/fedda8c8-51ba-4dc4-a842-29979e71d4a8.png" 
              alt="Logo školy" 
              className={`object-contain ${settings.isFullscreen ? 'w-[512px] h-[512px]' : 'w-96 h-96'}`}
            />
          </div>
        </div>
        
        <div className="flex-1 text-center">
          <h1 className={`font-bold leading-tight ${settings.isFullscreen ? 'text-7xl' : 'text-6xl'}`}>
            Vozovna Motol
          </h1>
          <div className="flex items-center justify-center gap-4 mt-2">
            <i className={`fa-solid fa-person-walking text-blue-100 animate-pulse ${settings.isFullscreen ? 'text-3xl' : 'text-2xl'}`}></i>
            <div className={`text-blue-100 ${settings.isFullscreen ? 'text-3xl' : 'text-2xl'}`}>
        ...
        <div className="flex flex-col items-end gap-6">
          {settings.showWeatherInHeader && (
            <div className={`${settings.isFullscreen ? 'text-6xl' : 'text-5xl'}`}>
              <WeatherHeader lat={50.0755} lon={14.4378} />
            </div>
          )}
          <div className="text-right">
            <div className={`font-bold ${settings.isFullscreen ? 'text-8xl' : 'text-7xl'}`}>
              {currentTime.toLocaleTimeString('cs-CZ')}
            </div>
            <div className={`text-blue-100 mt-2 ${settings.isFullscreen ? 'text-4xl' : 'text-3xl'}`}>
  ...
  const renderLeftHeader = (station: any) => (
    ...
    >
      ...
      <div className="px-6 py-4 relative z-10 h-full flex items-center">
          ...
          <div className="flex items-center justify-between w-full">
              ...
              <div className="flex-shrink-0 cursor-pointer" onClick={handleLogoClick}>
                <img 
                  ...
                  className={`object-contain ${
                    settings.vozovnaOnlyMode && settings.splitView ? 
                      (settings.isFullscreen ? 'w-[512px] h-[512px]' : 'w-96 h-96') :
                      (settings.isFullscreen ? 'w-56 h-56' : 'w-48 h-48')
                  }`}
                />
              </div>
              <div className="flex-1 text-center">
            ...
            <div className="flex flex-col items-end gap-4">
              ...
              <div className="flex items-center gap-4 text-center">
                <i className={`fa-solid fa-person-walking text-blue-100 animate-pulse ${settings.isFullscreen ? 'text-5xl' : 'text-4xl'}`}></i>
                <div>
                  <div className={`font-semibold ${settings.isFullscreen ? 'text-4xl' : 'text-3xl'}`}>Motol</div>
                  <div className={`text-blue-100 ${settings.isFullscreen ? 'text-3xl' : 'text-2xl'}`}>6 min • 400m</div>
                </div>
              </div>
            </div>
          </div>
  ...
  if (settings.splitView) {
    ...
    if (settings.vozovnaOnlyMode) {
      ...
      return (
        <div 
          className={`bg-gradient-to-br from-blue-50 via-white to-amber-50 flex overflow-hidden h-screen`}
          style={{ 
            transform: `scale(${settings.zoomLevel})`,
            transformOrigin: 'top left',
            width: `${100 / settings.zoomLevel}%`,
    ...
    return (
      ...
      >
        <div className="flex-1 flex flex-col h-full">
          {renderLeftHeader(currentMotolStation)}
          <div className={`flex-1 ${settings.isFullscreen ? 'p-1' : 'p-2'} overflow-hidden`}>
            <div 
        ...
        <div className="flex-1 flex flex-col h-full">
          {renderRightHeader(currentVozovnaStation)}
          <div className={`flex-1 ${settings.isFullscreen ? 'p-1' : 'p-2'} overflow-hidden`}>
            <div 
              className={`h-full transition-all duration-800 ease-in-out ${isTransitioning ? 'opacity-0 transform translate-x-8' : 'opacity-100 transform translate-x-0'}`}
              style={{ fontSize: `${effectiveTextSize}em` }}
      ...
      >
        ...
        <div className="px-6 py-4 relative z-10 h-full flex items-center justify-between">
          <div className="flex items-center gap-6">
            <div className="flex-shrink-0 cursor-pointer" onClick={handleLogoClick}>
              <img 
                src="/pictures/fedda8c8-51ba-4dc4-a842-29979e71d4a8.png" 
                alt="Logo školy" 
                className={`object-contain ${settings.isFullscreen ? 'w-80 h-80' : 'w-72 h-72'}`}
              />
            </div>
            <div className="flex-1 text-center">
              <h1 className={`font-bold leading-tight ${settings.isFullscreen ? 'text-7xl' : 'text-6xl'}`}>
          ...
          <div className="flex items-center gap-4 text-center">
            <i className={`fa-solid fa-person-walking text-blue-100 animate-pulse ${settings.isFullscreen ? 'text-5xl' : 'text-4xl'}`}></i>
            <div>
              <div className={`font-semibold ${settings.isFullscreen ? 'text-4xl' : 'text-3xl'}`}>
                Motol
              </div>
              <div className={`text-blue-100 ${settings.isFullscreen ? 'text-3xl' : 'text-2xl'}`}>6 min • 400m</div>
            </div>
